 ...
 *
 * `summons/99999/chibi.png` 与 `characters/KOTAMA/chibi.png` 是仓库里**仅有的两张跟进版本控制的
 * 官方美术**：根 `.gitignore` 把 `resources/ba` 下的两层目录一律排除，只给它们开了例外，
 * 因为拉不到又没有公开来源，不跟进去换台机器就永远缺。clone 下来它们已经在了，不用手工传。
 * （这里不写成通配路径 —— 那个写法里的星号斜杠会把块注释提前闭合掉。）
 *
 ...
 *
 * `summons/field/chibi.png` 是项目原创的场地路障，不在下载清单里，也不能用 99999 覆盖。
 */
