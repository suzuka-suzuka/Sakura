...
console.log("=== 0. 战报只保留实际行动 ===")
check("回合头的当前 Cost 与『过』都不进入战报",
  mergeTurnLog([
    "--- 蓝方回合（Cost 3）---",
    "[蓝] 过",
    "[蓝] 星野 普通攻击",
...
{
  check("开局伤害曲线：1~3 轮 50%，4~7 轮逐轮 +10%，8 轮起 100%",
    Array.from({ length: 9 }, (_, i) => roundDamageScale(i + 1)),
    [0.5, 0.5, 0.5, 0.6, 0.7, 0.8, 0.9, 1, 1])

  const directDamageAt = (round) => {
    const st = setup(["野宫", "野宫", "野宫", "野宫"], ["星野", "星野", "星野", "星野"])
  ...
  const fullDirect = directDamageAt(8)
  check("直接伤害实际应用开局倍率：第 8 轮约为第 1 轮两倍",
    Math.abs(fullDirect - protectedDirect * 2) <= 1, true)
  ...
  // 第 1 轮伤害为 50%：治疗先抬满，再扣 15 + 20 = 965。
  ...
  check("持续治疗先抬满、开局 DoT 按 50% 后最终 965", survived.state.sides[0].units[0].hp, 965)
  ...
  check("持续治疗先救起，再承受开局 50% DoT",
    [rescued.state.sides[0].units[0].alive, rescued.state.sides[0].units[0].hp], [true, 115])

  // 治疗后仍不够扛住全部 DoT 时照常死亡；治疗事件必须先出现。
  const dead = timed(50, [
    { icon: "Burn", amount: 80 },
    { icon: "Poison", amount: 60 },
  ], 10)
  ...
  // 进行中的旧对局仍是「无伤害参数的 field + 人身上的 Zone DoT」；升级后要照旧结算，不能双跳或崩档。
  {
    ...
    check("旧存档的单位 Zone DoT 继续结算且服从开局 50%，旧 field 只负责圈的倒计时",
      [ev?.totalAmount, field?.turns, field?.icon ?? null], [5, 1, null])
  }
