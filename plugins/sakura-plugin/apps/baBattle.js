...
function renderGuideFallback() {
  return [
    ...
    "",
    "出招：星野ex　/　过",
    "一律只写释放者名字，不用编号，也不能追加目标名；玩家决定谁放 EX，落点由技能描述和战场位置自动决定。",
    "技能自带的最低血量、最高生命、最高攻击等索敌优先；没有特殊描述时，主力按战场与前中后排锁定。",
    "支援对敌技能默认前排 → 中排 → 后排；同时有伤害和减益时按伤害算（如小玉普技，减攻跟随受击者）。给队友的进攻拐默认后排 → 中排 → 前排；治疗默认己方佩洛洛 → 前排 → 中排 → 后排，加防 / 闪避 / 护盾等防御技能也默认前排 → 中排 → 后排，同排都取号位小的。",
    "支援的普通技能和 EX 都不被嘲讽改点，但会执行集火。",
    "小春 EX 自动攻击敌人，同时治疗攻击主目标对位的存活友军；没有存活对位就不治疗。泉奈 EX 朝自动攻击目标移动一格，阵亡队友的位置也能进入。",
    "一次只放一个 EX。放完会先出图，还能再放就继续写；发「过」才结算普通技能和普攻、交给对方。",
    "只有在你确实有 EX 能放的时候才会 @ 你。一个都放不出来（Cost 不够 / 全在冷却 / 全员被嘲讽或恐惧）时机器人直接替你过，战报和战场图照发。",
    "普攻与 EX 都按固定规则锁定目标。放过 EX 的人本回合不再放小技能；换弹类（鹤城 / 芹香）本回合仍普攻，跟普攻阶段一起结算。",
    ...
    "多目标技能从自动主目标向外扩散；同战场只剩一人时范围技当场退化成单体。「敌方全体」不受此限。",
    "嘲讽和集火只替换攻击主目标 / 范围中心，不会强制把范围技压成单体。要求同身位的圆扇仍按新主目标的身位筛选溅射目标；爱丽丝普攻 / EX 这类直线则无论锁到谁都贯穿该目标所在整个战场，桃、纯子 EX 仍以新目标为中心铺三路。",
    "佩洛洛是比前排更前的独立身位：白子、睦月、千世与柚子普攻等同身位范围先锁到它后只打佩洛洛；晴奈 EX、爱丽丝普攻 / EX 的两路直线与纯子、桃 EX 的三路直线无视身位，范围内的佩洛洛会作为额外目标。全体、随机、绿的顺序点名和响的多圆也会算上它，千世 EX 的地面场地会同时烧佩洛洛、学生和掩体。千世、柚子的范围普攻是 Block=0，会无视掩体。",
