...
function renderGuideFallback() {
  return [
    "【碧蓝档案 · 回合制群战】4v4 + 每方 2 支援",
    "角色数值、技能与战斗公式全部照搬原作（等级1 / 无装备 / 统一3★ / 技能1级）。",
    "",
    "一方完成一次行动叫 1 回合；先手、后手各完成 1 回合叫 1 轮。每个实际结算步骤都发战报转发 → 战场图；只有接下来确实有可放 EX 的玩家才会收到 @，连续自动过掉的中间回合不 @。",
    "开局伤害保护：第 1～3 轮所有角色伤害为 50%，第 4～7 轮依次为 60% / 70% / 80% / 90%，第 8 轮起恢复 100%；治疗、护盾与当前生命自损不受影响。",
    "指令都可以不加 #。",
    "档案对战 @某人　发起（不 @ 则公开邀战）",
    "应战　　　　　　接受",
    ...
    "配队顺序就是左起站位，决定普攻对位。",
    "",
    "出招：星野ex打白子　/　芹娜ex给椿　/　小春ex奶野宫　/　日奈ex　/　过",
    "一律写角色名，不用编号。需要角色落点的单体 / 范围 EX 必须点名；全体、随机、循环、自身类不需要目标，只能直接写「角色名ex」，多写目标也会拦截。",
    "「打」指定敌方，「给 / 奶 / 换」指定友方；通常没写中间字时由技能类型判断阵营。小春两边都能选，必须明确写「打」或「奶 / 给」。需要点名却没写目标时只会提示，不会自动释放。",
    "普通技能的最低血量、最高生命、最高攻击等索敌优先；没有特殊描述时，主力按战场与前中后排锁定。",
    "支援的自动普通技能：对敌默认前排 → 中排 → 后排；给队友的进攻拐默认后排 → 中排 → 前排；治疗默认己方佩洛洛 → 前排 → 中排 → 后排，防御技能默认前排 → 中排 → 后排。需要落点的支援 EX 仍由玩家点名。",
    "小春 EX 有两种写法：打敌方时以所点敌人为伤害中心，并治疗其正对位的存活友方；奶友方时治疗所点友方，只在其正对面有存活敌人时结算现有范围伤害。两边都不向旁边补对位目标。泉奈 EX 点名相邻队友换位，也能进入阵亡队友留下的位置。",
    "一次只放一个 EX。放完会先出图，还能再放就继续写；发「过」才结算普通技能和普攻、交给对方。",
    "只有在你确实有 EX 能放的时候才会 @ 你。主力与支援合计一个都放不出来（Cost 不够 / 全在冷却 / 受控制）时机器人直接替你过；战报和战场图照发，但文字战报不写「过」或当前 Cost。主力全被嘲讽时，支援若仍有可放 EX 就会停下来等你。",
    "普攻与普通技能按固定规则锁定目标；需要落点的 EX 以玩家点名为主目标。放过 EX 的人本回合不再放小技能；换弹类（鹤城 / 芹香）本回合仍普攻。",
    ...
    "嘲讽最高，其次同战场的佩洛洛，再是前 → 中 → 后。两个前排就还是 2 打 2；前+中的多目标只打前排。被嘲讽的那一轮放不出 EX；净化只解实际目标一人，她立刻恢复 EX 与正常索敌，其他队友仍被嘲讽。",
    "",
    "【范围】原作的扇形/圆形按覆盖面积折算成打 2 / 3 / 全体，范围伤害不衰减，每个目标吃全额。",
    "多目标 EX 从玩家点名的主目标向外扩散；不需要点名的技能继续按自身规则选目标。同战场只剩一人时范围技会退化成单体，「敌方全体」不受此限。",
    "点名只改变主目标 / 范围中心，不改变技能形状。要求同身位的圆扇仍按主目标身位筛选；爱丽丝普攻 / EX 这类直线仍贯穿主目标所在整个战场，桃、纯子 EX 仍以所点目标为中心铺三路。",
    "佩洛洛是比前排更前的独立身位，也能被点作大范围 EX 的主目标。白子、睦月等同身位范围以它为中心时会退化成单体；晴奈 EX、爱丽丝普攻 / EX、纯子与桃 EX 的直线仍保留原范围，圈内学生照常受伤。全体、随机、绿的顺序点名和响的多圆也会算上它。",
    "场地固定留在原号位，每次跳伤害重新扫描当前站位：移入就受伤，移出就不再受伤。",
    "",
    "【分段】原作的多段攻击原样保留，每一段独立判定命中与暴击。",
    "段数越多伤害越稳，单段技能则是要么全中要么全空。",
    "",
    `Cost：每个本方回合结束时，每名存活主力与每名支援各回复 ${CFG.COST_REGEN_PER_UNIT}，满编共 +${6 * CFG.COST_REGEN_PER_UNIT}，上限 ${CFG.COST_MAX}；不足 1 点的小数会累计。`,
    `开局行动前不预支回费：先手首轮 ${CFG.COST_START} 点，后手首轮 ${CFG.COST_START + CFG.SECOND_BONUS} 点；实际差值之后会受 EX 消耗和技能回费影响。`,
    "满编时显示四名主力 + 两名支援的六张 EX 卡，阵亡主力的卡会移除；一次只放一个。能否释放只看存活、Cost、冷却和控制，没有额外的「同一角色每回合最多一次」限制；支援不受嘲讽。",
    "EX 冷却长度 = 当前存活施放者总数（含支援）− 3，最低为 0；4 / 3 / 2 / 1 名主力存活时，分别要等本方再放 3 / 2 / 1 / 0 个 EX。",
    "只剩 1 名主力 + 2 名支援时冷却为 0，Cost 够可以连续放同一角色。任一主力阵亡后，轮到本方时全队（含支援）EX 冷却清空。",
    "",
    `【时长】普通技能冷却按持有者自己的回合结束递减；状态时长按真正生效的攻击窗口计：进攻 / 治疗向状态与控制跟自己方回合，防御向状态、护盾、不死、嘲讽与召唤物跟敌方回合。1 轮 = ${CFG.ROUND_SECONDS} 秒。`,
    "周期型普通技能开局压满冷却：CD5 在本方前 5 回合结束后归零，最早于本方第 6 回合行动阶段触发；血量条件类技能满足条件才触发，部分每场限用几次。",
    "命中失败只取消该段伤害，技能的附加效果仍然生效。",
    "Buff / Debuff 从施放瞬间生效；同一原作槽位（Channel）后放覆盖先放，不比较强弱，不同槽位才共存，并按属性规则计算（百分比乘算、固定值相加）；没有槽位时才按施加者区分。",
    "护盾是真血上方的独立蓝色假血条，重复施加以最后一次盾量和持续时间为准。",
    "战场图用黄底 ! 表示普通技能已就绪；阵亡角色变灰，不显示 KO。当前 Cost 只看战场图，文字战报不重复播报。",
    "",
    ...
    "",
    "【目标】需要角色落点的 EX 必须点名；不需要单一落点的 EX 不能追加目标。佩洛洛可成为合法目标，也会参与生命值判定与治疗。",
    "",
...
export class BaBattle extends plugin {
   ...
   *
   * 嘲讽 / 恐惧只会封住场上主力，支援位仍可放 EX；因此只有
   * `exCastableOf` 把主力和支援全部算完后仍为空，才能确定玩家确实没有选择。
   *
   ...
   * 自动过掉的回合照样展示实际行动；战报格式化层会过滤「过」和回合头的当前 Cost。
   */
