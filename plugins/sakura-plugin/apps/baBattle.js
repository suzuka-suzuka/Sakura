...
function renderGuideFallback() {
  return [
    ...
    "",
    "出招：星野ex　/　过",
    "一律只写释放者名字，不用编号，也不能追加目标名；玩家决定谁放 EX，落点由技能描述与战场位置自动决定。",
    "技能自带的最低血量、最高生命、最高攻击等索敌优先；没有特殊描述时，主力按战场、前中后排、嘲讽、集火与召唤物规则锁定。",
    "支援对敌技能默认前排 → 中排 → 后排；给队友的进攻拐默认后排 → 中排 → 前排；治疗默认己方佩洛洛 → 前排 → 中排 → 后排，防御技能默认前排 → 中排 → 后排。支援的普通技能和 EX 都不被嘲讽改点，但会执行集火。",
    "小春 EX 自动攻击敌人，同时治疗攻击主目标正对位的存活友方；没有存活对位就不治疗。泉奈 EX 朝自动攻击目标移动一格，也能进入阵亡队友留下的位置。",
    ...
    "普攻、普通技能与 EX 都按固定规则锁定目标。放过 EX 的人本回合不再放小技能；换弹类（鹤城 / 芹香）本回合仍普攻。",
    ...
    "多目标技能从自动主目标向外扩散；同战场只剩一人时范围技会退化成单体，「敌方全体」不受此限。",
    "嘲讽和集火只替换自动主目标 / 范围中心，不改变技能形状。要求同身位的圆扇仍按新主目标身位筛选；爱丽丝普攻 / EX 这类直线仍贯穿该目标所在整个战场，桃、纯子 EX 仍以新目标为中心铺三路。",
    "佩洛洛是比前排更前的独立身位。白子、睦月等同身位范围先锁到它时会退化成单体；晴奈 EX、爱丽丝普攻 / EX、纯子与桃 EX 的直线仍保留原范围，圈内学生照常受伤。全体、随机、绿的顺序点名和响的多圆也会算上它。",
    ...
    "",
    "【目标】EX 不能指定别人，只能选择释放者。技能自带选人描述优先，其余目标按战场位置确定；佩洛洛会参与生命值判定与治疗。",
    "",
