...
- `#转账`（钓鱼 Lv.5 开放）、`#领取复活币`（无需当日钓鱼；Lv.1 为 20，每级 +2，余额低于同额时每日可领一次）。
- `#钓鱼`、`#装备鱼竿`、`#装备鱼饵`、`#装备鱼线`。
- 鱼竿与鱼线都是唯一装备：同一型号最多持有 1 件，不会像鱼饵或道具一样堆叠。
- `#钓鱼状态`（已整合鱼雷与鱼价状态）、`#钓鱼图鉴`（旧命令 `#钓鱼记录` 仍可用）、`#投放鱼雷`、`#钓鱼排行`。
- `#钓点`（钓点一览与解锁条件）、`#前往钓点 钓点名`（切换钓点，随钓鱼等级逐步解锁，不同钓点出没的鱼不同，未标注钓点的鱼全地图通用）。
- `#钓鱼图鉴 钓点名` 可筛选查看该钓点的图鉴。
- 每个钓点都有一名专属首领。所有钓点宝箱均有 5% 概率开出非卖品「首领鱼饵」；装备后发送 `#钓鱼`，必定挑战当前钓点的首领。
- 首领沿用普通渔获的重量判定，通过后跳过普通难度分支直接进入 120 秒单人首领战。发送「拉」控制距离、「溜」控制张力、「攻」削减生命；「攻」冷却 5 秒，必须同时把首领生命和距离降到 0。
- 六名首领均采用传说级以上的重量、难度、生命和攻击数值，强弱不按钓点解锁顺序递增，而由各自数值侧重与机制形成差异。
