...
- `#转账`（钓鱼 Lv.5 开放）、`#领取复活币`（无需当日钓鱼；Lv.1 为 35，每级 +2，余额低于同额时每日可领一次）。
- `#钓鱼`、`#装备鱼竿`、`#装备鱼饵`、`#装备鱼线`。
...
- 每个钓点都有一名专属首领。六种宝箱均有 5% 概率开出通用的非卖品「首领鱼饵」；在任意钓点装备后，下一竿均会 100% 呼出当地首领。
- 首领沿用普通渔获的重量判定，通过后跳过普通难度分支直接进入 100～110 秒单人首领战。发送「拉」控制距离、「溜」控制张力、「攻」削减生命；「攻」冷却 5 秒，必须同时把首领生命和距离降到 0。
- 六名首领按强度、装备风险与金币奖励逐级上升，以传说之竿和秘银线作为毕业挑战基准；星穹御灵竿是大毕业后的高价金币回收装备，不参与首领平衡基准。
- 首领每 5 秒攻击鱼线与鱼竿：两者损伤都会永久保存；任一装备耐久归零都会立即断裂并失去当前这一件。六种独立机制分别为绯樱鲤王抽取体力、青柳猿王持续偷钱、雾甲鳄龟冲高张力、赤潮锯鲨皇撕裂鱼线、海沟吞舟兽重创鱼竿、星渊古龙持续回血。当前仅支持单人挑战，多人协作暂未开放。
- 每个钓点都有一则专属“怪谈”噩梦；抽到噩梦稀有度后，当地怪谈占 50%，其余通用噩梦合计占 50%。
- 六则怪谈拥有独立机制：池底花嫁施加冥婚红线、柳下水猴偷取鱼饵、雾中捞尸人夺走倒影并拖往其他钓点、暮潮幽灵船留下收益债务、叩舱潜水钟施加额外体力消耗、食星之影吞噬限时 Buff。
- 普通诅咒与冥婚红线并存时优先消耗诅咒；红线会暂停并保留层数，待诅咒耗尽后继续生效。
- `#开宝箱 [宝箱名]`：开启钓点专属宝箱。宝藏稀有度渔获固定为当前钓点的专属宝箱（如樱花池塘的「花瓣木箱」、星辉秘境的「星屑圣匣」），钓上后放入背包，开箱按 `resources/economy/treasure_chest.yaml` 的掉落表随机开出金币、鱼饵、鱼线、道具或珍宝；随机鱼线只会抽取尚未拥有的型号，全部集齐后该权重自动并入普通樱花币。只持有一种宝箱时可省略名称。
- 六种宝箱的直接期望价值基本相同：低阶箱更稳定并保留小概率惊喜，高阶箱则把更好的道具和金币集中在较低概率档位。
- 宝箱中的限时钓鱼 Buff 持续 35 分钟；按结算后 5 分钟冷却设计，正常操作可覆盖约 5～6 竿。
- 普通单次垂钓在最大体重、天气、鱼雷、双倍卡和商人加成全部叠加后，直接获得的金币仍不超过 2000；该口径不扣除鱼饵、装备耐久等成本，异色渔获与首领不受此上限目标约束。
- `#职业列表`、`#选择职业`、`#进阶职业`。
