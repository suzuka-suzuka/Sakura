...
- 首领沿用普通渔获的重量判定（好运护符可跳过），通过后跳过普通难度分支直接进入 60 秒单人首领战。「拉」「溜」与普通精准溜鱼完全使用同一套数值；「攻」按当前有效鱼竿控制力造成伤害，冷却 5 秒。必须同时把首领生命和距离降到 0。
- 六名首领统一为困难度 230、重量 170～210、生命 180、攻击 8，仅特殊机制不同。以满体力、满耐久传说之竿（控制力 200）和秘银线、平均每秒精准操作一次为无增益/减益基准，普通首领模拟胜率约为 80%；异色首领及控制力损失、深压等状态另行影响实战难度。
...
- 通用噩梦会损伤鱼竿、永久削减当前鱼竿控制力、偷取金币或施加五层骷髅诅咒；骷髅诅咒每层会把本竿全部宝藏权重转成噩梦。
- 所有噩梦仍会扯断当前鱼线；河神垂青只保住鱼线，深渊猎手的猎魔守护则会消耗1次储存并完整免疫本次断线、伤害、偷取与附加状态。
- 六则怪谈拥有独立机制：溺水花嫁令噩梦权重连乘翻倍（×2、×4、×8……，直至净化）、河童水猴偷走背包中价值最高的鱼饵、雾中捞尸人按当前体力损伤鱼竿并把体力强制变为 1、暮潮幽灵船施加收益减半后再偿还的债务、叩舱潜水钟施加令鱼竿控制力减半的无上限深压、食星之影随机吞噬当前鱼竿以外的背包物品。
- 噩梦权重严格按“花嫁连乘 → 骷髅诅咒转移宝藏 → 怪物诱饵 +50 → 雾灯将垃圾与噩梦归零”结算。净化圣水会清除花嫁倍率、骷髅诅咒、幽灵船债务与深压，但不会恢复鱼竿永久控制力损失。
...
- `#职业列表`、`#选择职业`、`#进阶职业`。钓鱼大师按当前鱼竿熟练度提供无上限鱼线承重（1级×0.25、2级×0.5）；深渊猎手1级最多储存1次完整噩梦免疫并每24小时恢复1次，2级最多储存2次并每12小时恢复1次。充能与下次恢复时间会显示在 `#钓鱼状态` 中。
