...
const THUMB_CACHE_DIR = path.join(plugindata, "pixivTemp")
...
const MAX_VERTICAL_IMAGES_PER_CANVAS = MAX_COLUMNS * MAX_ROWS_PER_CANVAS // 单画布可容纳的最大竖图数
const MAX_LAYOUT_SHIFT_CELLS = MAX_VERTICAL_IMAGES_PER_CANVAS - 1 // 最大前置空位（达到后保持）
...
/**
 * 获取本次排行榜一览图的版式偏移（用于避免重复图完全一致）
 * - 每次请求同榜单会按 1 格递增，最大为「单画布最大竖图数-1」
 * - 达到最大偏移后保持该值，直到 key 在 11 点过期
 * - 偏移按“格子”计算，不改图内容，仅改版式
 */
async function getRankingLayoutShift(modeKey = '日榜', rankDate = '') {
  try {
    const redis = getRedis()
    const dayKey = rankDate || (() => {
      const n = new Date()
    ...
    const key = `sakura:pixiv:ranking:layout_shift:${modeKey}:${dayKey}`
    const counter = await redis.incr(key)
    if (counter === 1) {
      await redis.expire(key, getExpireSeconds())
    }

    return Math.min(MAX_LAYOUT_SHIFT_CELLS, Math.max(0, Number(counter - 1) || 0))
  } catch (error) {
    logger.warn(`[P站排行榜] 获取版式偏移失败，使用默认布局: ${error.message}`)
    return 0
  }
}
 ...
 */
function calculateLayout(illusts, leadingEmptyCells = 0) {
  const canvases = []
  let cursor = Math.max(0, Number(leadingEmptyCells) || 0)
  ...
  for (let i = 0; i < illusts.length; i++) {
    ...
    let globalRow = Math.floor(cursor / MAX_COLUMNS)
    let col = cursor % MAX_COLUMNS

    // 横图若放不下则换到下一行
    if (col + colSpan > MAX_COLUMNS) {
      cursor += (MAX_COLUMNS - col)
      globalRow = Math.floor(cursor / MAX_COLUMNS)
      col = cursor % MAX_COLUMNS
    }

    const canvasIndex = Math.floor(globalRow / MAX_ROWS_PER_CANVAS)
    const row = globalRow % MAX_ROWS_PER_CANVAS

    if (!canvases[canvasIndex]) {
      canvases[canvasIndex] = []
    }

    canvases[canvasIndex].push({
      illust,
      index: i,  // 全局排名序号
      row,
      col,
...
async function generateRankingImages(illusts, config, modeKey = "日榜", rankDate = '') {
  const layoutShiftCells = await getRankingLayoutShift(modeKey, rankDate)
  const layouts = calculateLayout(illusts, layoutShiftCells)
  const images = []
