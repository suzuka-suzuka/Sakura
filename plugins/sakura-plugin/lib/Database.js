...
class DB {
  ...
  migrate() {
    ...
    // 鱼雷重新放宽为「每人每个钓点一枚」：曾经用来锁死「每人一枚」的唯一索引必须删掉，
    // 约束回落到主键 (group_id, user_id, location) 上，库层照样挡得住并发重复投放。
    this.db.exec('DROP INDEX IF EXISTS idx_pond_torpedoes_user_unique;');
