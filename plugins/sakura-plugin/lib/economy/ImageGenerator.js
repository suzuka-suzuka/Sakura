...
export default class EconomyImageGenerator {
  ...
  drawWrappedTitle(ctx, text, x, y, maxWidth, lineHeight) {
    const lines = this.wrapText(ctx, String(text || ""), maxWidth).slice(0, 2)
    ...
    if (lines[1]) {
      ...
      ctx.fillText(lines[1], this.width / 2, y + lineHeight)
    }
  ...
  async generateTransactionImage(data) {
    ...
    const measureCanvas = createCanvas(this.width, 10)
    const measureCtx = measureCanvas.getContext("2d")
    measureCtx.font = 'bold 38px ZhuZiAYuan, "MotoyaMaru", "Noto Color Emoji", "Noto Sans SC", sans-serif'
    const titleLines = this.wrapText(measureCtx, String(data.title || ""), this.width - 92).slice(0, 2)
    const headerHeight = titleLines.length > 1 ? 192 : 150
    const rowHeight = 78
    ...
    const titleBottomY = this.drawWrappedTitle(ctx, data.title, 46, 62, this.width - 92, 42)

    ctx.fillStyle = "#999999"
    ctx.font = '20px ZhuZiAYuan, "MotoyaMaru", "Noto Color Emoji", "Noto Sans SC", sans-serif'
    ctx.textAlign = "center"
    ctx.fillText(data.subtitle, this.width / 2, titleBottomY + 36)
    ctx.textAlign = "left"
    ...
    const timeX = tableX + colWidth / 2
    const actionX = tableX + colWidth * 1.5
    const amountX = tableX + colWidth * 2.5
    ...
    ctx.fillText("事项", actionX, tableY + 25)
    ...
    records.forEach((item, index) => {
      ...
      ctx.font = '22px ZhuZiAYuan, "MotoyaMaru", "Noto Color Emoji", "Noto Sans SC", sans-serif'
      ctx.fillText(this.truncateText(ctx, item.action, colWidth - 34), actionX, y + 42)
  ...
  async generateTransactionAnalysisImage(data) {
    ...
    measureCtx.font = 'bold 38px ZhuZiAYuan, "MotoyaMaru", "Noto Color Emoji", "Noto Sans SC", sans-serif'
    const titleLines = this.wrapText(measureCtx, String(data.title || ""), this.width - 92).slice(0, 2)
    const titleOffset = titleLines.length > 1 ? 42 : 0
    const height = 650 + titleOffset + categories.length * 58
    const canvas = createCanvas(this.width, height)
    const ctx = canvas.getContext("2d")
    ...
    ctx.font = 'bold 38px ZhuZiAYuan, "MotoyaMaru", "Noto Color Emoji", "Noto Sans SC", sans-serif'
    const titleBottomY = this.drawWrappedTitle(ctx, data.title, 46, 62, this.width - 92, 42)
    const contentOffset = titleBottomY - 62
    ...
    const cardY = 125 + contentOffset
    ctx.fillStyle = "rgba(255, 255, 255, 0.88)"
    ctx.shadowColor = "rgba(255, 105, 180, 0.15)"
    ctx.shadowBlur = 10
    this.drawRoundedRect(ctx, cardX, cardY, cardW, 160, 18)
    ctx.fill()
    ctx.shadowBlur = 0
    ...
    const chartX = 70
    const chartY = 340 + contentOffset
    ...
    ctx.fillText("流水分类", 50, 480 + contentOffset)

    const categoryMax = Math.max(...categories.map(item => item.total), 1)
    categories.forEach((item, index) => {
      const y = 515 + contentOffset + index * 58
