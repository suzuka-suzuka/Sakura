...
class ImageEmbeddingManager {
  ...
  async detectImageMimeType(imageBuffer, fallback = "image/png") {
    try {
      const metadata = await sharp(imageBuffer).metadata();
      return this.getMimeTypeFromFormat(metadata.format, fallback);
    } catch (error) {
      logger.warn(`[ImageEmbedding] Failed to detect image MIME: ${error.message}`);
      return fallback;
    }
  }

  resolveStoredImagePath(row) {
    if (!row) return null;
    ...
    if (row.file_name) {
      candidates.push(path.join(EMOJI_IMAGES_DIR, row.file_name));
    }
    if (row.hash) {
      for (const ext of [".png", ".gif", ".jpg", ".jpeg", ".webp"]) {
        candidates.push(path.join(EMOJI_IMAGES_DIR, `${row.hash}${ext}`));
      }
    }
    ...
    return row.file_name
      ? path.join(EMOJI_IMAGES_DIR, row.file_name)
      : row.file_path || null;
  }

  repairRowFilePath(row) {
    if (!row) return null;

    const resolvedPath = this.resolveStoredImagePath(row);
    if (!resolvedPath) {
      return row;
    }

    if (row.file_path !== resolvedPath) {
      try {
        db.prepare("UPDATE image_metadata SET file_path = ? WHERE id = ?").run(
          resolvedPath,
          row.id
        );
        row.file_path = resolvedPath;
      } catch (error) {
        logger.warn(`[表情向量] 修复文件路径失败: ${error.message}`);
      }
    }
  ...
  buildImageItem(row, extra = {}) {
    const repairedRow = this.repairRowFilePath({ ...row });
    return {
      id: repairedRow.id,
      hash: repairedRow.hash,
      localPath: repairedRow.file_path,
      filename: repairedRow.file_name,
      description: repairedRow.description,
      metadata: JSON.parse(repairedRow.metadata || "{}"),
      ...extra,
    };
  }

  repairStoredImagePaths() {
    try {
      const rows = db
        .prepare(
          "SELECT id, hash, file_path, file_name, description, metadata FROM image_metadata"
        )
        .all();
      let repairedCount = 0;
      ...
      for (const row of rows) {
        const resolvedPath = this.resolveStoredImagePath(row);
        if (!resolvedPath || resolvedPath === row.file_path) {
          continue;
        }
      ...
      if (repairedCount > 0) {
        logger.info(`[表情向量] 已修复 ${repairedCount} 条迁移后的文件路径`);
      }
    } catch (error) {
      logger.warn(`[表情向量] 批量修复文件路径失败: ${error.message}`);
    }
  }
  ...
  async describeImage(imageBuffer, mimeType = "image/png") {
    // Convert GIF to PNG before sending it to the vision model.
    let processedBuffer = imageBuffer;
    let processedMimeType = await this.detectImageMimeType(imageBuffer, mimeType);
    if (processedMimeType.includes("gif")) {
      try {
  ...
  async checkImage(imageUrl) {
    ...
    if (existing) {
      const item = this.buildImageItem(existing);
  ...
  async addPreparedImage(fileInfo, metadata = {}) {
    ...
    const fallbackMimeType = filename.endsWith(".gif") ? "image/gif" : "image/png";
    const mimeType = await this.detectImageMimeType(imageBuffer, fallbackMimeType);
  ...
  async searchImage(query, topK = 1, minScore = 0) {
    ...
    const queryEmbedding = await this.generateTextEmbedding(query, "task: search result | query: ");
    ...
    for (const result of results) {
      ...
      if (row) {
        const item = this.buildImageItem(row, {
  ...
  async deleteImage(id) {
    ...
    if (row) {
      const repairedRow = this.repairRowFilePath({ ...row });
      if (repairedRow.file_path && fs.existsSync(repairedRow.file_path)) {
        try {
          fs.unlinkSync(repairedRow.file_path);
        } catch (err) {
  ...
  getAll() {
    ...
    return rows.map(row => this.buildImageItem(row));
  }
  ...
  async findLeastSimilar(keyword, count = 20) {
    ...
    const queryEmbedding = await this.generateTextEmbedding(keyword, "task: search result | query: ");

    const allResults = await this.index.queryItems(queryEmbedding, keyword, totalCount);
    ...
    for (const result of leastSimilar) {
      ...
      if (row) {
        enrichedResults.push(this.buildImageItem(row, {
          similarity: result.score
        }));
      }
    }
