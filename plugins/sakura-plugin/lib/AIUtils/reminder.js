...
import { getBot, withBotContext } from '../../../../src/api/client.js';
...
let removeOnceReminderListener = null;
...
export function addRepeatReminderTask({ targetQQ, groupId, content, cronExpression, selfId = null }) {
  const reminderConfig = Setting.getConfig('reminderTask', { selfId }) || {};
  const tasks = Array.isArray(reminderConfig.tasks) ? [...reminderConfig.tasks] : [];

  const taskId = buildNextReminderTaskId(tasks);
  tasks.push({
    id: taskId,
    enable: true,
    cron: cronExpression,
...
function buildNextReminderTaskId(tasks) {
...
export async function scheduleOnceReminder({ delayMs, targetQQ, groupId, content, selfId = null }) {
  await initializeOnceReminderListener();
...
export async function initializeOnceReminderListener() {
  if (onceReminderListenerReady) return removeOnceReminderListener;

  const unsubscribe = await onRedisKeyExpired(async (expiredKey) => {
    ...
    try {
      ...
      await withBotContext(task.selfId, async () => {
        const currentBot = task.selfId ? getBot(task.selfId) : (typeof bot !== 'undefined' ? bot : null);
  ...
  removeOnceReminderListener = () => {
    unsubscribe();
    onceReminderListenerReady = false;
    removeOnceReminderListener = null;
  };
  return removeOnceReminderListener;
}
