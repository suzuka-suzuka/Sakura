...
import { getRecentGroupMessages } from "./groupMessageStore.js";
...
function formatStoredChatMessage(record) {
  const senderId = record.userId;
  const senderName = record.senderName || senderId || "未知用户";
  const senderRole = roleMap[record.senderRole] || "普通成员";
  let messageHeader = `【${senderName}】(QQ:${senderId}, 角色:${senderRole}`;

  if (record.senderTitle) {
    messageHeader += `, 头衔:${record.senderTitle}`;
  }
  ...
  if (repliedMessage) {
    const originalSenderId = repliedMessage.userId;
    const originalSenderName =
      repliedMessage.senderName || originalSenderId || "未知用户";
    const originalContent = String(repliedMessage.contextContent || "")
      .trim()
      .replace(/\n/g, " ");
  ...
  messageHeader += `) 说：`;
  const messageContent = String(record.contextContent || "");
  return `${messageHeader}${messageContent}`;
}
...
export async function buildGroupPrompt(groupId, options = {}) {
  ...
  try {
    const storedMessages = await getRecentGroupMessages({
      selfId: bot.self_id,
      groupId,
      limit: groupContextLength,
    });
    if (storedMessages.length >= groupContextLength) {
      formattedChats = storedMessages.map(formatStoredChatMessage);
      contextSource = "redis";
    }
  } catch (storeError) {
    logger.warn(`[GroupContext] 读取 Redis 群消息失败，回退 API: ${storeError.message}`);
  }

  if (formattedChats.length === 0) {
    try {
      const chats = await getChatHistoryGroup(group, groupContextLength);
      formattedChats = await Promise.all(
