  ...
  func = async function (opts, e) {
    ...
    try {
      ...
      if (action === "fine") {
        ...
        const fineSuccess = economyManager.transferCoins(targetE, botE, fineAmount);
        if (!fineSuccess) {
          return `罚款失败：对方余额发生变化，请稍后再试。`;
        }
      ...
      if (action === "transfer") {
        ...
        // master 特权：无视限额，直接转账最多能转的金额
        if (e.isMaster) {
          const actualAmount = Math.min(transferAmount, botCoins);
          if (actualAmount <= 0) {
            return `转账失败：余额不足。`;
          }
        ...
        const rejectAsScammer = async () => {
          ...
          const fineSuccess = economyManager.transferCoins(targetE, botE, fineAmount);
          if (!fineSuccess) {
            return `警告：${targetNickname}(${e.user_id}) 是骗子！但罚款时余额发生变化，未能完成扣款。立即停止转账，不要再尝试。`;
          }
