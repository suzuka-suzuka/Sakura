  ...
  parameters = {
    properties: {
      tag: {
        ...
        description:
          "图片的日文标签列表。请将标签翻译为日文，但 ‘可爱’ 是特殊标签，无需翻译直接使用",
      },
      isR18: {
        type: "boolean",
  ...
  func = async function (opts, e) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Lolicon API 请求失败: ${response.status} - ${errorText}`
        );
      }
      ...
      const filteredTags = imageInfo.tags.filter(
        (tag) => !tagsToFilter.includes(tag)
      );
