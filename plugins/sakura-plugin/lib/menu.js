...
export const menuGroups = [
  {
    ...
    items: [
      ...
      { name: "取消流程", command: "#取消", desc: "取消当前正在进行的人设或语音角色录入流程。" },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      { name: "添加词条", command: "#添加词条 <关键词>", desc: "引用消息添加关键词回复。", costCommand: charge("KeywordReply.添加词条"), examples: ["引用一条消息后发送 #添加词条 早安"] },
      { name: "删除词条", command: "#删除词条 <关键词>", desc: "删除当前群关键词回复。", costCommand: charge("KeywordReply.删除词条") },
      ...
      { name: "主动拟态回复", command: "@机器人 / 配置触发词", desc: "艾特机器人或命中触发词时按拟态回复扣费；普通概率被动回复不扣费。", costCommand: charge("Mimic.Mimic"), trigger: "主动" },
    ],
  },
  {
    ...
    items: [
      ...
      { name: "我的资产", command: "#我的资产", desc: "查看当前樱花币余额和签到等级。" },
      { name: "流水", command: "#流水 / #查流水 [页码]", desc: "查看自己的樱花币收支图，每页最多 20 条；主人可 @用户 查看。" },
      { name: "流水分析", command: "#今日流水分析 / #本周流水分析", desc: "生成当前流水的收支统计图和分类可视化。" },
      { name: "转账", command: "#转账 <数量> @用户", desc: "签到 Lv.3 后可向其他用户转账樱花币。", costNote: "需签到 Lv.3；转账金额" },
      { name: "发红包", command: "#发红包 <金额> <个数> [均等|拼手气] [祝福语]", desc: "签到 Lv.3 后可发红包；金额和个数都必填。", costNote: "需签到 Lv.3；本金加手续费" },
      { name: "抢红包", command: "#抢红包", desc: "抢群里尚未领完的红包。" },
      { name: "出售", command: "#出售 <物品>", desc: "出售背包物品；钓具按原价80%回收，鱼竿另按剩余耐久折价。" },
      { name: "使用物品", command: "#使用 <物品> [品质]", desc: "使用背包中的特殊物品；星愿瓶需要指定品质。", costNote: "消耗物品" },
      { name: "道具重铸", command: "#道具重铸 <道具> [次数|全部]", desc: "每2件同名特殊道具随机重铸为1件异名特殊道具。", costNote: "每次消耗2件" },
      ...
      { name: "等级奖励", command: "#领取等级奖励", desc: "钓鱼每5级一档、无等级上限，每档发放修理工具箱和净化圣水各1个，可跨档补领。" },
      { name: "图鉴奖励", command: "#领取图鉴奖励 [钓点名]", desc: "按各钓点专属鱼的收录数发档（10/20/30/全收录），发放该钓点宝箱、专属道具与藏品；不填钓点则结算全部钓点。" },
  ...
  },
  {
    ...
    items: [
      { name: "钓鱼", command: "#钓鱼", desc: "进行一次钓鱼；装备首领鱼饵时必定引出当前钓点首领。", costNote: "受装备/道具影响" },
      ...
      { name: "钓鱼状态", command: "#钓鱼状态", desc: "查看等级、职业、装备、鱼雷与钓鱼状态。" },
      { name: "鱼塘天气", command: "#鱼塘天气", desc: "只能观测当前天气；下一时段不可预报，具体影响需要自行摸索。" },
      { name: "钓鱼图鉴", command: "#钓鱼图鉴 [钓点名]", desc: "查看钓鱼图鉴与收集进度；指定钓点时只列该钓点专属鱼，进度口径与图鉴奖励一致。" },
      ...
      { name: "投放鱼雷", command: "#投放鱼雷", desc: "在当前钓点永久投放鱼雷，每个钓点各限一枚，直到被其他玩家钓中或自行引爆。", costNote: "消耗鱼雷" },
      { name: "引爆鱼雷", command: "#引爆鱼雷 [钓点名]", desc: "引爆自己投放且已过引信时间的鱼雷；不填钓点则引爆当前钓点的那枚。" },
      ...
      { name: "进阶职业", command: "#进阶职业", desc: "达到要求的钓鱼等级后进阶当前职业。", costNote: "需达到钓鱼等级" },
    ],
  },
  {
    id: "group", title: "群管理", desc: "成员清理、禁言、踢人、公告和群申请。",
    items: [
      { name: "清理从未发言", command: "#清理从未发言的人", desc: "列出从未发言的成员，确认后批量清理。", permission: "管理/白名单" },
      { name: "清理长期未发言", command: "#清理7天未发言的人 / #清理1个月未发言的人", desc: "列出超过指定时间未发言的成员，确认后批量清理。", permission: "管理/白名单" },
      { name: "清理低等级", command: "#清理低于10级的人", desc: "列出群等级低于指定等级的成员，确认后批量清理。", permission: "管理/白名单" },
      ...
      { name: "群申请处理", command: "#开门 <门牌号> / #关门 <门牌号>", desc: "处理入群申请。", permission: "管理/白名单" },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "群聊洞见", command: "#群聊洞见 [今天/昨天/前天/日期] [刷新]", desc: "生成群聊统计、关系图谱、行为标签、热门话题和金句报告。" },
      ...
      { name: "早安", command: "早安", desc: "记录起床时间并计算睡眠。" },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "群聊日报", command: "每天23:59", desc: "为当天达到消息门槛的群生成不使用缓存的群聊洞见日报。", trigger: "定时" },
