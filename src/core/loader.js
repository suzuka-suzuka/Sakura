...
function getEventLogContext(e) {
  ...
  const eventType = [e.post_type, e.message_type || e.notice_type || e.request_type || e.meta_event_type]
    .filter(Boolean)
    .join(".");
  const groupPart = e.group_id ? `[�?${e.group_id}]` : "[私聊]";
...
async function checkAndConsumeCoins(e, instance, handler) {
  try {
    ...
    logger.error(`[Loader] 检查指令消耗出�? ${err}`);
    return true;
  }
}
...
export class PluginLoader {
  ...
  async loadPlugins() {
    ...
    this.sortHandlers();
    logger.info(`共加�?${this.executableHandlers.length} 个指�?事件处理器`);
  ...
  _registerConfigWatcher(pluginName) {
    ...
    pluginConfigManager.onChange(pluginName, async (configName, newConfig) => {
      ...
      if (pluginPaths && pluginPaths.size > 0) {
        logger.info(`[Loader] 配置 ${key} 变更，正在重�?${pluginPaths.size} 个相关插�?..`);
        ...
        debounceTimer = setTimeout(async () => {
          try {
            ...
            if (isInAppsDir) {
              ...
              logger.info(`检测到插件变更: ${filename}，正在重�?..`);
              ...
              logger.info(`检测到文件变更: ${filename}，正在重�?..`);
    ...
    return logContext.run(_logCtx, () => eventStorage.run(eventObj, async () => {
      ...
      if (context) {
        ...
        try {
          ...
          logger.error(`${getEventLogContext(e)} 插件 ${plugin.name} 上下文执行出�? ${err}`);
        }
      }
